...
function LanguageSelector({ hasAccess }: { hasAccess: boolean }) {
  ...
  // List of languages available to free users
  const availableForFreeUsers = ["javascript", "c", "python", "java", "sql"];
  ...
  const handleLanguageSelect = (langId: string) => {
    // Allow selection for languages in availableForFreeUsers or if user has access
    if (!hasAccess && !availableForFreeUsers.includes(langId)) return;
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <motion.button
        ...
        className={`group relative flex items-center gap-3 px-4 py-2.5 bg-[#1e1e2e]/80 rounded-lg transition-all duration-200 border border-gray-800/50 hover:border-gray-700 ${!hasAccess && !availableForFreeUsers.includes(language) ? "opacity-50 cursor-not-allowed" : ""}`}
      >
        <div
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            ...
            <div className="max-h-[280px] overflow-y-auto overflow-x-hidden scrollbar-thin scrollbar-thumb-gray-700 scrollbar-track-gray-800">
              {Object.values(LANGUAGE_CONFIG).map((lang, index) => {
                const isLocked =
                  !hasAccess && !availableForFreeUsers.includes(lang.id);
